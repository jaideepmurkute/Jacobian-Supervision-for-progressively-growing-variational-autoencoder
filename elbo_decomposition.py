...
def elbo_decomposition(vae, teacher_vae, dataset_loader, args):
    ...
    print("N - number of data samples: ", N)
    q_dist = dist.Normal()
    ...
    with torch.no_grad():
        for data in tqdm(dataset_loader):
            ...
            xs = xs.view(batch_size, 3, args.H, args.W).cuda()
            z_mean, z_logsigma = vae.encode_for_elbo_decomp(xs)  # (batch_size, z_dim, 2)
            qz_params[n:n + batch_size, :, 0] = z_mean
            qz_params[n:n + batch_size, :, 1] = z_logsigma
            n += batch_size
    ...
    qz_samples = q_dist.sample(params=qz_params_expanded) #[48w,2,1]
