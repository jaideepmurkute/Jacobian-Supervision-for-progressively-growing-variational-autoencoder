...
from utils import min_max_scaling, get_kl_divergence, get_reconstruction_error, compute_elbo_loss, imgs2gif, make_gif, \
    mutual_info_metric_shapes
import copy
...
batch_size = 128  # changed from 128   # The size of input data took for one iteration
...
model_name = 'unsupervised_teacher_dsprite3'
...
def get_data_loader(dataset_type, load_all_files=False):
    ...
    # dataset = Dsprites_dataset(test=is_test, dir='data/', size=(H, W), all_files=load_all_files)
...
if __name__ == "__main__":
    ...
    if choice == 1:
        ...
        # train_loader = get_data_loader(dataset_type='train', load_all_files=True)
        model, optimizer = get_model(read_checkpoint=True, checkpoint_path=MODEL_SAVE_DIR + '/' + model_name + '.pt')
        # data_loader = get_data_loader(dataset_type='train', load_all_files=True)
        data_loader = getShapesLoader('./data', batchsize=batch_size, use_cuda=True, dataset_type='whole_mig')

        #avg_total_loss, avg_recon_loss, avg_kld_loss, hidden_mean, hidden_sigma = \
        #    test_model(model, data_loader, hidden_size, 'train', plot_latent_dist=True)
        #print("Loss values for the dsprites dataset: ")
        #print(
