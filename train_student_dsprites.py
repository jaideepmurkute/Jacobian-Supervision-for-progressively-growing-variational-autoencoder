...
from dataloader_dsprites import Dsprites_dataset, getShapesLoader
from new_arch_models_dsprites import autoencoder
from utils import *
...
parser.add_argument('C', default=1, nargs='?', type=int, help='Number of channels in input image')
...
parser.add_argument('hidden_size', default=6, nargs='?', type=int, help='Size of the latent layer in VAE')
parser.add_argument('teacher_hidden_size', default=4, nargs='?', type=int, help='Size of the latent '
...
parser.add_argument('load_from_checkpoint', default=False, nargs='?', type=bool,
                    help='Load pretrained model for further training')
parser.add_argument('use_jacobian_supervision', default=False, nargs='?', type=bool,
...
use_jacobian_supervision = False
lambda_z = 1.0
lambda_jacobian = 1.0
lambda_xcov = 0.0001

BASE_SAVE_DIR = './shapes_new_arch_unsupervised'
model_name = 'unsupervised_student_dsprite6_jac'  # Student model name
teacher_model_name = 'unsupervised_student_dsprite3_jac'
...
def get_other_losses(student_model, teacher_model, img, student_output, z, lambda_z, lambda_xcov, lambda_jacobian):
    ...
    student_nuisance_z = z[:, :-args.teacher_hidden_size]
    student_factor_z = z[:, -args.teacher_hidden_size:]
    # print("student_factor_z.size(): ", student_factor_z.size())

    loss_z = torch.mean((teacher_z - student_factor_z) ** 2)
    factor_retention_loss = loss_z * args.lambda_z  # cost for same factor z prediction
    # ============================= xcov loss ===========================================================
    ...
    xcov_loss = loss_xcov * args.lambda_xcov
    ...
    jacobian_loss = jacobian_loss * args.lambda_jacobian
...
def get_data_loader(dataset_type, load_all_files=False):
    ...
    # dataset = Dsprites_dataset(dir='./data', test=True, size=(args.H, args.W))
    # data_loader = torch.utils.data.DataLoader(dataset, batch_size=args.batch_size,
    #                                           shuffle=shuffle_flags[dataset_type], num_workers=0,
    #                                           pin_memory=True)
    data_loader = getShapesLoader('./data', batchsize=batch_size, use_cuda=True)
    ...
    if plot_flag:
        ...
        for target_z_index in range(n_z):
            ...
            for ri in range(gif_nums + 1):
                ...
                reconstr_img = reconstr_img.squeeze(1)  # to convert (N, 1, 64, 64) -> (N, 64, 64)
                rimg = reconstr_img[0, :, :].permute(1, 2,
                                                        0)  # change this line for dsprites, to have (64, 64) output - without channels
                samples.append(rimg)
...
if __name__ == "__main__":
    ...
    if choice == 1:
        ...
        data_loader = getShapesLoader('./data', batchsize=batch_size, use_cuda=True, dataset_type='whole_mig')
        # evaluate_model(model=model, data_loader=data_loader)
        # print('-' * 50)

        # print("Computing MIG metric ...")
        # metric, marginal_entropies, cond_entropies = mutual_info_metric_dsprite(vae=model, dataset_loader=data_loader, nparams=2,
        #                                                                           K=args.hidden_size)
        # print("MIG metric on {} dataset: {:.5f}".format('Whole', metric))
        
        print("Traversing latent space for visualization ...")
        select_dim = disentangle_check_image_row_dsprite_z(model, data_loader, args, PLOTS_SAVE_DIR, dataset_type='dsprites')
        print("select_dim: ", select_dim)
        print('-'*50)
        # del data_loader
        #print("Computing Factor-VAE score ...")
        #dataset = Dsprites_dataset(root='./data')
        #fac_metric = factor_metric_dsprite(dataset='dsprite', dataset_reference=dataset)
